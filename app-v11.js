...
console.info("FITLYNE app-v11 ativo", { build: C.BUILD, api: FITLYNE_API_URL });
...
function productStatus(product) {
  let value = String(product.STATUS_CATALOGO || "AUTOMATICO").toUpperCase();
  if (!STATUS_OPTIONS.some(([key]) => key === value)) value = "AUTOMATICO";
  if (value === "AUTOMATICO") value = Number(product.ESTOQUE_ATUAL || 0) > 0 ? "DISPONIVEL" : "ESGOTADO";
  if (value === "DISPONIVEL" && Number(product.ESTOQUE_ATUAL || 0) <= 0) value = "ESGOTADO";
  return value;
}
...
function renderDashboard() {
  ...
  const stock = active.reduce((total, product) => total + Number(product.ESTOQUE_ATUAL || 0), 0);
  const low = active.filter((product) => Number(product.ESTOQUE_ATUAL || 0) <= Number(product.ESTOQUE_MINIMO || 0));
...
function productPhoto(id) {
  const photos = state.photos.filter((photo) => photo.ID_PRODUTO === id);
...
function renderProducts() {
  ...
  $("#productList").innerHTML = products.map((product) => {
    ...
    const rawSelected = String(product.STATUS_CATALOGO || "AUTOMATICO").toUpperCase();
    const selected = STATUS_OPTIONS.some(([value]) => value === rawSelected) ? rawSelected : "AUTOMATICO";
    return `<article class="product-card">
      <img loading="lazy" decoding="async" src="${productPhoto(product.ID)}" alt="${escapeHtml(product.NOME)}">
      <div class="product-card-body">
        ...
        <p>Estoque interno: <b>${product.ESTOQUE_ATUAL}</b></p>
        <label class="quick-status-label">Status no catálogo
          <select class="quick-status" onchange="updateProductStatus('${product.ID}', this.value, this)">${quickStatusOptions(selected)}</select>
        </label>
...
window.updateProductStatus = async function updateProductStatus(id, status, select) {
  const oldValue = state.products.find((product) => product.ID === id)?.STATUS_CATALOGO || "AUTOMATICO";
...
async function uploadImage(file, productId, index) {
  ...
  const cloudName = String(result.cloud_name || C.CLOUDINARY_CLOUD_NAME);
  ...
  const overlay = C.CLOUDINARY_WATERMARK_PUBLIC_ID
    ...
    : "";
  const makeUrl = (width, height, crop = "fill", quality = "auto:eco") => `${deliveryBase}f_auto,q_${quality},c_${crop},w_${width},h_${height}/${overlay}${version}${publicId}.${format}`;
  return {
    ID: uid("FOTO"), ID_PRODUTO: productId, ORDEM: index + 1, PRINCIPAL: index === 0 ? "SIM" : "NAO", PUBLIC_ID: publicId,
    URL_ORIGINAL: result.secure_url,
    URL_CATALOGO: makeUrl(720, 900),
    URL_FEED: makeUrl(1080, 1350, "fill", "auto:good"),
    URL_STORY: makeUrl(1080, 1920, "fill", "auto:good"),
    URL_WHATSAPP: makeUrl(1080, 1350, "fill", "auto:good"),
    URL_FACEBOOK: makeUrl(1200, 1500, "fill", "auto:good"),
    URL_SHOPEE: makeUrl(1200, 1200, "pad", "auto:good"),
    URL_MERCADO_LIVRE: makeUrl(1200, 1200, "pad", "auto:good")
  };
}
...
window.editProduct = function editProduct(id) {
  ...
  const oldPhotos = state.photos.filter((photo) => photo.ID_PRODUTO === id);
  if (oldPhotos.length) {
    $("#photoPreview").innerHTML = oldPhotos.map((photo) => `<div class="photo-preview existing-photo"><img loading="lazy" src="${photo.URL_CATALOGO || photo.URL_ORIGINAL}" alt="Foto publicada"><span>Publicada</span></div>`).join("");
  }
  toggleSizeMode();
...
window.deleteProduct = async function deleteProduct(id) {
  ...
  try {
    ...
    state.photos = state.photos.filter((photo) => photo.ID_PRODUTO !== id);
    renderProducts();
    renderDashboard();
